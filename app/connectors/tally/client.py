...
class TallyClient:
    ...
    def presync_unit(self, name: str, symbol: str = "") -> bool:
        """
        ...
        Named distinctly from sync_unit(data) below (the full, mapping-tracked "units"
        sync entity's sync_func) — this is a lightweight, untracked, best-effort helper
        called only from equipment.py's _presync_units, not run_sync_pipeline.
        """
        clean_name = (name or "").strip()
        if not clean_name:
            return False
        ...
        if self.check_exists("unit", clean_name):
            return False
        company_name = getattr(self.cfg, "tally_company_name", None)
        xml = build_unit_xml({"name": clean_name, "symbol": symbol}, action="Create", company_name=company_name)
