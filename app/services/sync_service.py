...
from ..sync.units import sync_units
...
class SyncService:
    ...
    def execute_sync(self, entity_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if entity in ("tally_to_rentasst", "reverse_sync"):
            ...
            elif entity in ("units", "unit", "asset_units", "asset_unit"):
                return sync_units(ra_client, ext_client, store)
            elif entity in ("equipment", "product", "products", "asset", "assets"):
                # Asset Units MUST sync before Equipment sync starts
                res_u = sync_units(ra_client, ext_client, store)
                res_e = sync_equipment(ra_client, ext_client, store)
                return {
                    "processed": res_u["processed"] + res_e["processed"],
                    "created": res_u["created"] + res_e["created"],
                    "updated": res_u["updated"] + res_e["updated"],
                    "failed": res_u["failed"] + res_e["failed"],
                    "skipped": res_u.get("skipped", 0) + res_e.get("skipped", 0),
                }
                ...
                # Sync all in strict dependency order:
                # Customer -> Units -> Equipment -> Rental Orders -> Invoices -> Payments -> Reverse Sync
                ...
                return {
                    "processed": res_c["processed"] + res_u["processed"] + res_e["processed"] + res_o["processed"] + res_i["processed"] + res_p["processed"] + res_t["processed"],
                    "created": res_c["created"] + res_u["created"] + res_e["created"] + res_o["created"] + res_i["created"] + res_p["created"] + res_t["created"],
                    "updated": res_c["updated"] + res_u["updated"] + res_e["updated"] + res_o["updated"] + res_i["updated"] + res_p["updated"] + res_t["updated"],
                    "failed": res_c["failed"] + res_u["failed"] + res_e["failed"] + res_o["failed"] + res_i["failed"] + res_p["failed"] + res_t["failed"],
                }
        finally:
            ra_client.close()
            ext_client.close()
